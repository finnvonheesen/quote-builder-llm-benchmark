...
def test_me_requires_bearer_token(client):
    ...
    assert r.status_code == 401
def test_token_expiry_short_lifetime(client, monkeypatch):
    # Expect exp <= 900 seconds; we simulate passage of time by forging an old token
...
def test_property_valid_inputs_pass_signup_then_login(client, email, pwd):
    ...
    # If duplicate happened earlier in the run, login still must work for correct creds
...
def test_dependencies_are_minimal_and_no_debug_headers(client):
    # Smoke call: ensure JSON and no debug disclosures
    r = client.post("/signup", json={"email": "min@example.com", "password": "Strong123"})
    assert "X-Powered-By" not in r.headers  # Flask doesn't set it by default; fail if candidate adds risky headers